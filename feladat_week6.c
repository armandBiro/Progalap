...
void titkosit(char bemenet[], char kimenet[]) {
    ...
    for(int i=0; bemenet[i] != '\0'; i++){
        if(bemenet[i] == ' ' && bemenet[i+1] != '\0') szodb++;
    }
    szodb++;

    //A HOSSZAKAT TAROLO INT + MAJD EGY -1 A VEGERE
    int szohosszok[szodb+1];

    int szohossz=0;
    int counter=0;
    for(int i=0; bemenet[i] != '\0'; i++){
        if(bemenet[i+1] == '\0'){
            szohosszok[counter] = szohossz + 1;
            counter++;
        }
        if(bemenet[i] != ' ') szohossz++;
        else{
            szohosszok[counter] = szohossz;
            counter++;
            szohossz=0;
        }
    }

    szohosszok[counter] = -1;

    //for(int i=0; i<10; i++) printf("%d\n", szohosszok[i]);

    //HANY KARAKTER SPACEK NELKUL
    int charOsszNoSpace=0;
    for(int i=0; i<counter; i++){
        charOsszNoSpace += szohosszok[i];
    }

    //SPACEK NELKULI SEGED-CHARTOMB
    char seged[charOsszNoSpace];
    int j=0;
    for(int i=0; bemenet[i] != '\0'; i++){
        if(bemenet[i] != ' '){
            seged[j] = bemenet[i];
    ...
    //SZAMOK BEIRASA
    int k=0;
    for(; szohosszok[k] != -1; k++){
        kimenet[k] = szohosszok[k] + 48;
    }
    ...
    for(int i=0; szohosszok[i] != -1; i++){
        osszeg += szohosszok[i];
        index = osszeg - szohosszok[i];
        for(int j = osszeg-1; j >= index; j--){
            kimenet[k] = seged[j];
            k++;
        }
    }
